...
export const GameProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const startNewGame = useCallback(async (cityName: CityStateName, government: GovernmentType) => {
    ...
    try {
      ...
      const initialEvent: GameEvent = {
        ...
        type: 'Political' as EventType,
        severity: 'Positive' as EventSeverity
      };
  ...
  const declareWar = useCallback((cityState: CityStateName) => {
    ...
    if (relationshipIndex !== -1) {
      updatedGame.relationships[relationshipIndex].status = 'War' as RelationshipStatus;
  ...
  const makePeace = useCallback((cityState: CityStateName) => {
    ...
    if (relationshipIndex !== -1) {
      updatedGame.relationships[relationshipIndex].status = 'Neutral' as RelationshipStatus;
  ...
  const establishTrade = useCallback((cityState: CityStateName) => {
    ...
    if (relationshipIndex !== -1) {
      ...
      if (updatedGame.relationships[relationshipIndex].status === 'War' as RelationshipStatus) {
        toast({
      ...
      if (!updatedGame.relationships[relationshipIndex].treaties.includes('Trade')) {
        ...
        if (updatedGame.relationships[relationshipIndex].status === 'Neutral' as RelationshipStatus) {
          updatedGame.relationships[relationshipIndex].status = 'Friendly' as RelationshipStatus;
        }
